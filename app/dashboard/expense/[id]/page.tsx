...
import { serverGetExpense, serverUpdateExpense } from "@/services/serverApi"
...
export default function EditExpensePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchExpense() {
      try {
        ...
        const expenseData = res?.data?.find((item: IExpense) => item._id === params.id)
  ...
  const initialValues = expense
    ? {
        captureDate: expense.captureDate ? new Date(expense.captureDate).toISOString().split("T")[0] : "",
        type: expense.type || "",
  ...
  const validationSchema = Yup.object({
    captureDate: Yup.date().required("Capture date is required"),
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await serverUpdateExpense(payload)
      if (res?.success) {
    ...
    return (
      <div className="flex items-center justify-center h-[50vh]">
        <div className="text-center">
          ...
          <p className="mt-2">Loading expense details...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="w-full">
      ...
      <Card className="w-full mx-auto">
        ...
        >
          {({ isSubmitting, values, setFieldValue }) => (
            <Form>
              <CardHeader>
                <CardTitle>Edit Expense Information</CardTitle>
      ...
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Confirm Expense Update</DialogTitle>
          </DialogHeader>
          <div>Are you sure you want to update this expense?</div>
          <DialogFooter>
            ...
            >
              {confirmLoading ? "Updating..." : "Yes, Update"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
